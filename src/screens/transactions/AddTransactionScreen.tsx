...
const AddTransactionScreen: React.FC<AddTransactionProps> = ({ navigation }) => {
  ...
  const handleSave = () => {
    addTransaction({
      title: selectedCategory || (transactionType === 'income' ? 'Income' : 'Expense'),
      amount: parseFloat(amount) || 100,
      category: selectedCategory || (transactionType === 'income' ? 'Income' : 'Other'),
      type: transactionType,
      accountId: selectedAccount || storeAccounts[0]?.id,
      notes,
      isRecurring,
